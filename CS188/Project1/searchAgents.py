...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i, c in enumerate(corners):
        if not vis[i]:
            h = max(h, util.manhattanDistance(c, pos))
    return h
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if n > 0:
        ...
        heap = []
        for nei in fg[1:]:
            d = util.manhattanDistance(fg[0], nei)
            ...
            if node not in visited:
                ...
                for nbr in fg:
                    if nbr not in visited:
                        d = util.manhattanDistance(node, nbr)
                        heapq.heappush(heap, (d, nbr))
    closest = min(util.manhattanDistance(position, food) for food in fg)
    return closest + total_cost
